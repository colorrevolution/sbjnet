...
def main():
    ...
    # D1 = torch.nn.DataParallel(Discriminator).cuda()
    # D2 = torch.nn.DataParallel(Discriminator).cuda()
    # vgg_features = torch.nn.DataParallel(Vgg19()).cuda()
    ...
    df_loss = torch.tensor(0)
    db_loss = torch.tensor(0)
    ...
    for step in tqdm(range(cfg.max_iter)):
        ...
        #   trainiter = iter(train_data)
        #   i_t, i_s, t_sk, t_t, t_b, t_f, mask_t = trainiter.next()
        for i_t, i_s, t_sk, t_t, t_b, t_f, mask_t in trainiter:
            i_t = i_t.cuda()
            i_s = i_s.cuda()
            t_sk = t_sk.cuda()
            t_t = t_t.cuda()
        ...
        writer.add_scalar("D_Loss/db_loss", db_loss.item(), step)
        writer.add_scalar("D_Loss/df_loss", df_loss.item(), step)
        if ((step + 1) % 2 == 0):
        ...
        #     print('Iter: {}/{} | Gen: {} | D_bg: {} | D_fus: {}'.format(step + 1, cfg.max_iter, g_loss.item(),
