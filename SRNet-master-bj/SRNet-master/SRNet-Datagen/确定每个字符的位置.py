...
def add_shelter(image,multi,shelter_rate_section):
    ...
    if np.random.random_sample() < 0.3:
        ...
        # 确定起始合并位置
        start_location = np.random.randint( length_multi - merge_number +1 )
    ...
    # 计算需要遮挡的字母数量
    shelter_character_number = int(shelter_character_rate * len(multi)) + 1
    ...
    for i in range(len(new_multi)):
        ...
        # try:
        #     img_hull = generate_poly(img, vertex_number, area_thresh)
        # except:
        #     # continue
        #     img_hull = generate_poly(img, vertex_number, area_thresh)

        # print("image[up:bottom, left:right]",image[up:bottom, left:right].shape)
...
for name in imageList:
    ...
    #自动选择字符遮挡区域
    print("multi", multi)
    shelter_image = add_shelter(image,multi,shelter_rate_section)
    cv2.imwrite("./test/"+name,shelter_image)





                # write_image = image[up:bottom,left:right]
                # write_image = write_image * img_hull
                # cv2.imwrite("./test/"+str(w)+".png",write_image)
                # print("up",up)
                # print("end",bottom)

                #
                # coordinate = str(left)+" "+ str(up)+" " + str(right) +" "+ str(bottom)
                # multi.append(coordinate)

    # coordinate_string =  "    ".join(multi) +"\n"   # 4个空格
    # file_txt.write(name +"        "+coordinate_string) # 8个空格
    # multi = []
