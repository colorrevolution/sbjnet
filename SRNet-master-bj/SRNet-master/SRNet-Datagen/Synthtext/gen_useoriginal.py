...
class datagen():
    ...
    def __init__(self):
        ...
        cur_file_path = os.path.dirname(__file__) # path of this file. os.path.dirname remove the file name ana retain the content.
        ...
        # print(" self.colorsRGB", self.colorsRGB[0])
        # print(" self.colorsLAB", self.colorsLAB[0])
        ...
        self.shelter_rate_section = data_cfg.shelter_rate_section
        self.number = 0
    ...
    def gen_srnet_data_with_background(self,classRate):
        ...
        while True:
            ...
            # for i in range(width):
            #     # 确定right的值
            #     if np.sum(image[:, width - 1 - i])!=0:
            #         right = width - 1 - i
            #         break
            #
            # if top - 5 >= 0 and bottom + 5 <= high and left - 5 >= 0 and right + 5 <= width:
            ...
            if bg_w < surf_w or bg_h < surf_h:
                ...
                print("running now")
                print("bg_h,bg_w",bg_h,surf_h,bg_w,surf_w)
            ...
            # bg_h, bg_w = bg.shape[:2]
            #
            # if bg_w < surf1.shape[0] or bg_h < surf1.shape[1]:
            #     self.number = self.number + 1
            #     print("running now")
            #     print("bg_h,bg_w", bg_h, surf_h, bg_w, surf_w)
            #
            ...
            # x = np.random.randint(0, bg_w - surf_w + 1)
            # y = np.random.randint(0, bg_h - surf_h + 1)
            # t_b = bg[y:y+surf1.shape[0], x:x + surf1.shape[1], :]
            ...
            _1 , surf_normal = render_standard_text.make_standard_text(self.standard_font_path, text1, (surf_h, surf_w))
            ...
            # surf1.flags.writeable = True
            # generate shelter
            surf1_old = surf1
            surf1 = np.array(surf1)
            # cv2.imshow("ddd",surf1)
            # cv2.waitKey(0)
            # random incomplete  probability=0.7
            if np.random.rand()<=0.6:
                surf1 , save_img_hull = generate_shelter.main_shelter(surf1,classRate)
            else:
                surf1 = surf1_old

            _, i_s = colorize.colorize(surf1, t_b, fg_col, bg_col, self.colorsRGB, self.colorsLAB, min_h, param)
            _, i_s_complete = colorize.colorize(surf1_old, t_b, fg_col, bg_col, self.colorsRGB, self.colorsLAB, min_h, param)
            ...
            # skeletonization
            # t_sk = skeletonization.skeletonization(surf_normal, 127)
            # bg[y:y + surf_h, x:x + surf_w, :] = i_s
            # full_image = bg
            t_t = _1
            break

        # return [i_t, i_s, t_sk, t_t, t_b, t_f, surf1,text1]
        # return [i_s, i_s,surf1,surf1, text1,save_img_hull[0]]
        # return [i_s, i_s_complete,surf1_old,surf1, text1,save_img_hull[0],fontstr]
        return [i_s,surf1]
...
#         self.queue = manager.Queue()  # multiple processes share queues
#         self.pool = multiprocessing.Pool(processes = self.process_num) #processes pool
#         self.processes = []
