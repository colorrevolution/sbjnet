...
def crop_safe(arr, rect, bbs=[], pad=0):
    rect = np.array(rect)
    #expend the rectangle by padding pixel
    rect[:2] -= pad
    rect[2:] += 2*pad
    # preventing the rectangle beyond the surface
    v0 = [max(0,rect[0]), max(0,rect[1])]
    v1 = [min(arr.shape[0], rect[0]+rect[2]), min(arr.shape[1], rect[1]+rect[3])]
    # intercept the appropriate reactangle for seting text
    arr = arr[v0[0]:v1[0], v0[1]:v1[1], ...]
    # update the relative size of the coordinates
    if len(bbs) > 0:
...
def render_normal(font, text):
    ...
    # font parameters:  the distance between two rows
    ...
    # print("lound", line_bounds)
    # round() 方法返回浮点数x的四舍五入值   fsize is the size of surface.
    ...
    bbs = [] # coordinate of each character
    space = font.get_rect('O')  # the width of character '0' is equal with  space.
    x, y = 0, 0
    for l in lines:
        x = 0 # carriage-return
        ...
        for ch in l: # render each character
            if ch.isspace(): # just shift
                ...
                # print("ch_bounds",ch_bounds,ch_bounds.x,ch_bounds.y,ch_bounds.width,ch_bounds.height)
                ch_bounds.x = x + ch_bounds.x
                ch_bounds.y = y - ch_bounds.y
                x += ch_bounds.width + 3
                bbs.append(np.array(ch_bounds))

    # print("bbs", bbs, type(bbs),len(bbs))
    # print("bbs[0]",bbs[0])
    # pdb.set_trace()
    # get the union of characters for cropping:
    # connect smaller rectangles into larger ones.
    ...
    bbs = np.array(bbs)
    # print("surf", type(surf), surf.shape)
    surf_arr, bbs = crop_safe(pygame.surfarray.pixels_alpha(surf), rect_union, bbs, pad=5)
    # print("surf_arr1", type(surf_arr), surf_arr.shape)
    surf_arr = surf_arr.swapaxes(0,1)
    # print("surf_arr2", type(surf_arr), surf_arr.shape)
    # pdb.set_trace()
