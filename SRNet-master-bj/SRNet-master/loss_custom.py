...
def build_discriminator_loss(x_true, x_fake):
    d_loss = -torch.mean(
        torch.log(torch.clamp(x_true, cfg.epsilon, 1.0)) + torch.log(torch.clamp(1.0 - x_fake, cfg.epsilon, 1.0)))
    return d_loss
...
def build_generator_loss(out_g, labels,out_vgg):
    o_sk, o_t= out_g
    t_sk, t_t = labels
    o_vgg = out_vgg
    # print("t_sk",t_sk.shape)
    ...
    l_t = l_t_l1 + l_t_sk +l_f_vgg_per + l_f_vgg_style
