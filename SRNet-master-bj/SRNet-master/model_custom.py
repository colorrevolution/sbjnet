...
class encoder_net(torch.nn.Module):
    ...
    def __init__(self, in_channels, get_feature_map=False):
        ...
        self._conv1_1 = Conv_bn_block(in_channels=in_channels, out_channels=self.cnum, kernel_size=3, stride=1,
                                      padding=1)
        ...
        # self._pool1 = torch.nn.Conv2d(in_channels=self.cnum, out_channels=2 * self.cnum, kernel_size=3, stride=2,
        #                               padding=calc_padding(temp_shape[0], temp_shape[1], 3, 2))
...
class decoder_net(torch.nn.Module):
    ...
    def __init__(self, in_channels, get_feature_map=False, mt=1, fn_mt=1):
        ...
        self._conv1_1 = Conv_bn_block(in_channels=fn_mt * in_channels, out_channels=8 * self.cnum, kernel_size=3,
                                      stride=1, padding=1)
        ...
        # -----------------
        #
        # self._deconv1 = torch.nn.ConvTranspose2d(8 * self.cnum, 4 * self.cnum, kernel_size=3, stride=2,
        #                                          padding=calc_inv_padding(temp_shape[0], temp_shape[1], 3, 2))
        self.upsample = torch.nn.UpsamplingNearest2d(scale_factor=2.0)
        self._deconv1 = Conv_bn_block(in_channels=8 * self.cnum, out_channels=4 * self.cnum, kernel_size=3, stride=1,
                                      padding=1)

        self._conv2_1 = Conv_bn_block(in_channels=fn_mt * mt * 4 * self.cnum, out_channels=4 * self.cnum, kernel_size=3,
                                      stride=1, padding=1)
        ...
        #                                          padding=calc_inv_padding(temp_shape[0], temp_shape[1], 3, 2))
        # self._deconv2 = torch.nn.UpsamplingNearest2d(scale_factor=2.0)
        self._deconv2 = Conv_bn_block(in_channels=4 * self.cnum, out_channels=2 * self.cnum, kernel_size=3, stride=1,
                                      padding=1)

        self._conv3_1 = Conv_bn_block(in_channels=fn_mt * mt * 2 * self.cnum, out_channels=2 * self.cnum, kernel_size=3,
                                      stride=1, padding=1)
        ...
        #                                          padding=calc_inv_padding(temp_shape[0], temp_shape[1], 3, 2))
        # self._deconv3 = torch.nn.UpsamplingNearest2d(scale_factor=2.0)
        self._deconv3 = Conv_bn_block(in_channels=2 * self.cnum, out_channels= self.cnum, kernel_size=3, stride=1,
                                      padding=1)
...
class text_conversion_net(torch.nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self._t_cbr = Conv_bn_block(in_channels=2 * self.cnum, out_channels=2 * self.cnum, kernel_size=3, stride=1,
                                    padding=1)
...
class Discriminator(torch.nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.cnum = 32
        self._conv1 = torch.nn.Conv2d(in_channels, 64, kernel_size=3, stride=2,
                                      padding=calc_padding(temp_shape[0], temp_shape[1], 3, 2))
        self._conv2 = torch.nn.Conv2d(64, 128, kernel_size=3, stride=2,
                                      padding=calc_padding(temp_shape[0], temp_shape[1], 3, 2))

        self._conv2_bn = torch.nn.BatchNorm2d(128)

        self._conv3 = torch.nn.Conv2d(128, 256, kernel_size=3, stride=2,
                                      padding=calc_padding(temp_shape[0], temp_shape[1], 3, 2))

        self._conv3_bn = torch.nn.BatchNorm2d(256)

        self._conv4 = torch.nn.Conv2d(256, 512, kernel_size=3, stride=2,
                                      padding=calc_padding(temp_shape[0], temp_shape[1], 3, 2))
...
if __name__ == '__main__':
    G = Generator(1)
    inp1 = torch.rand((2, 1, 64, 256))
    inp2 = torch.rand((2, 1, 64, 256))
    a,b = G(inp1,inp2,1)
    print(b.size())